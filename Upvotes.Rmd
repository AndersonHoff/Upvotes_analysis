...
```
It is important to identify missing values (NA's) on the data. The result obtained below tell us that there are not NA cells in the numeric columns.
...
Also, there is not a blank categorical cell in the TAG column, since the line below contain only letters. 
...
In order to observe how the data is spread in each column, some graphs  and tables were created. 
