...
if(!require(keras)) {install.packages("keras")} else {library(keras)}
```
...
rmse_nlm <- rmse(test_set$Upvotes, test_set$predict)
#3624  3108
```
...
For the XGBoost analysis, the RMSE for the testing sample is ` rmse_xgb`.
